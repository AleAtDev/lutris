...
class UbisoftConnectClient:
    def __init__(self, service):
        ...
        self._session.headers.update(
            {"Authorization": None, "Ubi-AppId": CLUB_APPID, "User-Agent": CHROME_USERAGENT, "Ubi-SessionId": None}
        )
    ...
    def authorise_with_stored_credentials(self, credentials):
        ...
        if not self.user_name or not self.user_id:
            ...
            user_data = {"username": self.user_name, "userId": self.user_id}
        self.post_sessions()
        self._service.store_credentials(self.get_credentials())
        return user_data
    ...
    def get_applications(self, spaces):
        ...
        return self._do_request_safe("get", f"https://api-ubiservices.ubi.com/v2/applications?spaceIds={space_string}")
    ...
    def post_sessions(self):
        headers = {"Content-Type": "application/json"}
        return self._do_request_safe(
            "post", "https://public-ubiservices.ubi.com/v2/profiles/sessions", add_to_headers=headers
        )
